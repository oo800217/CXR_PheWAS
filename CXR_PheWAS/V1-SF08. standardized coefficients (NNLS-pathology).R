...
PhecodeInfo = read.csv('data/PhecodeInfo.csv')
...
cor_df$Var1 <- factor(
  cor_df$Var1,
  levels = c(features_order, "Hold-out dataset", "External dataset", "MIMIC dataset")
)

levels(cor_df$Var1)[!levels(cor_df$Var1) %in% features_order] <-
  levels(cor_df$Var1)[!levels(cor_df$Var1) %in% features_order] %>%
  gsub(" dataset", "", .) %>%
  paste0("R-square (", ., ")")
...
# 4. 只改 Var2 的 level 名稱，不改順序
old_levels <- levels(cor_df$Var2)

phe_tmp <- str_extract(old_levels, "^[0-9]+\\.?[0-9]*")

desc_tmp <- PhecodeInfo$description[
  match(phe_tmp, as.character(PhecodeInfo$phenotype))
]

desc_tmp[is.na(desc_tmp)] <- phe_tmp[is.na(desc_tmp)]

new_levels <- paste0(
  desc_tmp,
  sub("^[0-9]+\\.?[0-9]*", "", old_levels)
)

new_levels <- new_levels %>%
  gsub("\\-", " (", .) %>%
  gsub("survival", "incident) [cluster ", .) %>%
  gsub("binary", "prevalent) [cluster ", .) %>%
  paste0(., AUC_data[!duplicated(AUC_data$y), "cluster"], "]")

levels(cor_df$Var2) <- new_levels
############
