...
for (m in 1:n_clusters.radio) {
  ...
  cluster_df.radio <- rbind(cluster_df.radio, data.frame(cluster = m,
                                                         start = min(which(radio_info[,'cluster'] %in% m)),
                                                         end = max(which(radio_info[,'cluster'] %in% m))))
  
}

cor_df.radio <- melt(cosine_sim.radio)
colnames(cor_df.radio) <- c("Var1", "Var2", "value")

cor_df.radio[,'Var1'] <- factor(cor_df.radio[,'Var1'], levels = features_order)
cor_df.radio[,'Var2'] <- factor(cor_df.radio[,'Var2'], levels = features_order)
...
cosine_p.radio <- ggplot(cor_df.radio, aes(x = Var1, y = Var2, fill = value))

cosine_p.radio <- cosine_p.radio + geom_tile(color = "white")
cosine_p.radio <- cosine_p.radio + scale_fill_gradientn(colors = my_colors, limits = c(-1, 1), name = "Cosine")

cosine_p.radio <- cosine_p.radio + labs(title = '', x = '', y = '')
cosine_p.radio <- cosine_p.radio + theme_minimal(base_size = 8)

cosine_p.radio <- cosine_p.radio + theme(plot.title = element_blank(),
...
for (i in 1:nrow(cluster_df.radio)) {
  ...
  cosine_p.radio <- cosine_p.radio + annotate(geom = "rect",
                                              xmin = cluster_df.radio[i,'start'] - 0.5, xmax = cluster_df.radio[i,'end'] + 0.5,
                                              ymin = cluster_df.radio[i,'start'] - 0.5, ymax = cluster_df.radio[i,'end'] + 0.5,
                                              color = "black",
                                              fill = NA, size = 0.5)
}

cosine_p.radio <- cosine_p.radio + coord_equal()
